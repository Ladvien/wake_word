...
class OpenWakeWordTrainer:
    ...
    def prepare_openwakeword_dataset(self):
        ...
        for i, src_file in enumerate(
            tqdm(positive_files, desc="Copying positive samples")
        ):
        ...
        for i, src_file in enumerate(
            tqdm(negative_files, desc="Copying negative samples")
        ):
    ...
    def train_with_openwakeword(self, data_dir):
        ...
        if not HAS_OPENWAKEWORD:
            raise ImportError(
                "OpenWakeWord is required for training. Install with: pip install openwakeword"
            )
        ...
        try:
            ...
            model_metadata = {
                ...
                "performance": {
                    "accuracy": (
                        model.history.get("accuracy", [])[-1]
                        if hasattr(model, "history")
                        else None
                    ),
                    "val_accuracy": (
                        model.history.get("val_accuracy", [])[-1]
                        if hasattr(model, "history")
                        else None
                    ),
                },
    ...
    def run_training_pipeline(self):
        ...
        try:
            ...
            if not self.positive_dir.exists() or not any(
                self.positive_dir.glob("*.wav")
            ):
